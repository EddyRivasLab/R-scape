...
static int AFR_calculate_insrate(FILE *fp, double *ret_rI, double *ret_muA, double *ret_ld, double gaposc, double gapesc, double betainf, double rM,
...
static int AFR_calculate_gapcosts(E1_RATE *R1, double time, ESL_ALPHABET *abc, E1_BG *bg, int mode, int L, double gapscale, double *ret_gape, double *ret_gapo, 
				  ...
				  double tol, char *errbuf, int verbose);
static int e1model_calculate_subrate(ESL_GETOPTS *go, ESL_ALPHABET *abc, E1_BG *bg, RATEBUILDER **ret_ratebld, int scaled, double tol,
	 ...
	 double betainf, double rM, int scaledrate, int N, double tol, char *errbuf, int verbose)
{
  ...
  double              tt;
  double              gapet, gapot;
  double              go1, go2, go3, go4, go5, go6;
  double              subsite;
  double              expsc;
  double              pid;
  double              totalt = 50.0;
  ...
  for (x = 0; x <= totalt*N; x ++) {
    ...
    status = AFR_calculate_gapcosts(R1, tt, abc, bg, mode, L, gapscale, &gapet, &gapot, &go1, &go2, &go3, &go4, &go5, &go6, tol, errbuf, FALSE);
    if (status != eslOK) { printf("%s\n", errbuf); esl_fatal(msg); }
    
    if (verbose) fprintf(stdout, "%f %f %f %f %f %f %f %f %f %f %f %f\n", tt, pid, 100*subsite, expsc, gapet, gapot, go1, go2, go3, go4, go5, go6);
		       ...
		       double tol, char *errbuf, int verbose)
{
  ...
  go6 = log((1.0-TMM)/2.0) + log(1.0 - TXX) - log(TXX);
