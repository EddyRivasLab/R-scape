 ...
 */
int
e2fhmmer_GForward(const PSQ *psql, const PSQ *psqr, const E2HMMER_PROFILE *gm, E2_GMX *gx, float *opt_sc)
{
  ...
  if (psql->n != psqr->n) { printf("e2fhmmer_GForward(): sqs are not aligned\n"); return eslFAIL; }

  /* reallocation, if needed */
  if ( (status = e2_gmx_GrowTo(gx, M, L, L)) != eslOK) return status;
  gx->M    = M;
  gx->L    = L;
  gx->type = E2_FORWARD;
  ...
  for (k = 1; k <= M; k ++) {
    ...
    if (x == L) {
      sc = p7_FLogsum(           IBMXM(x,k) + e2hmmerTSC(k,e2HP_IB_EE),
...
#if 0
    if (x==L)printf("e2fFWD x %d k %d/%d apair %d BB %f IB %f SS %f DS %f IS %f SD %f DD %f ID %f BI %f SI %f DI %f Ii %f iI %f\n", 
		    x, k, M, apair, 
 ...
 */
int
e2fhmmer_GBackward(const PSQ *psql, const PSQ *psqr, const E2HMMER_PROFILE *gm, E2_GMX *gx, float *opt_sc)
{
  float const **tsc = (float const **)gm->tsc;
  float       **dp  = gx->dp;						
  float         sc;
  ...
  EE = 0.0;
  /* BB state */
  BBMXM(x,k) = EE + e2hmmerTSC(k,e2HP_BB_EE);  
  /* IB state */
  IBMXM(x,k) = EE + e2hmmerTSC(k,e2HP_IB_EE);  
  /* SS state */
  SSMXM(x,k) = EE + e2hmmerTSC(k,e2HP_SS_EE);  
  /* DS state  */
  DSMXM(x,k) = EE + e2hmmerTSC(k,e2HP_DS_EE);	
  /* IS state  */
  ISMXM(x,k) = EE + e2hmmerTSC(k,e2HP_IS_EE);	
  /* SD state */
  SDMXM(x,k) = EE + e2hmmerTSC(k,e2HP_SD_EE);       
  /* DD state */
  DDMXM(x,k) = EE + e2hmmerTSC(k,e2HP_DD_EE);
  /* ID state  */
  IDMXM(x,k) = EE + e2hmmerTSC(k,e2HP_ID_EE);	
  /* BI state */
  BIMXM(x,k) = EE + e2hmmerTSC(k,e2HP_BI_EE);
  /* SI state */
  SIMXM(x,k) = EE + e2hmmerTSC(k,e2HP_SI_EE);	
  /* DI state  */
  DIMXM(x,k) = EE + e2hmmerTSC(k,e2HP_DI_EE);
  /* Ii state */
  IiMXM(x,k) = EE + e2hmmerTSC(k,e2HP_Ii_EE);
  /* iI state */
  iIMXM(x,k) = EE + e2hmmerTSC(k,e2HP_iI_EE);
#if 0
  if (x==91) printf("\ne2fBCW x %d  k %d BB %f IB %f SS %f DS %F IS %f SD %f DD %f ID %f BI %f SI %f DI %f Ii %f iI %f \n", x, k, 
	   BBMXM(x,k), IBMXM(x,k), SSMXM(x,k), DSMXM(x,k), ISMXM(x,k), SDMXM(x,k), 
	   DDMXM(x,k), IDMXM(x,k), BIMXM(x,k), SIMXM(x,k), DIMXM(x,k), IiMXM(x,k), iIMXM(x,k));
#endif    
  ...
  for (k = gm->M-1; k >= 0; k --) {
    ...
    SSMXM(x,k) = p7_FLogsum(EE  + e2hmmerTSC(k,e2HP_SS_EE), 
			    DDMXM(x,kv) + e2hmmerTSC(k,e2HP_SS_DD));
	
    /* DS state (6+1)-5=2 transitions */
    DSMXM(x,k) = p7_FLogsum(EE  + e2hmmerTSC(k, e2HP_DS_EE), 
    ...
    SDMXM(x,k) = p7_FLogsum(EE  + e2hmmerTSC(k, e2HP_SD_EE), 
			    DDMXM(x,kv) + e2hmmerTSC(k, e2HP_SD_DD));
    
    /* DD state (5+1)-4=2 transitions */
    DDMXM(x,k) = p7_FLogsum(EE  + e2hmmerTSC(k,e2HP_DD_EE), 
    ...
    if (x==0)printf("e2fBCW x %d  k %d BB %f IB %f SS %f DS %F IS %f SD %f DD %f ID %f BI %f SI %f DI %f Ii %f iI %f EE %f\n", x, k, 
	   BBMXM(x,k), IBMXM(x,k), SSMXM(x,k), DSMXM(x,k), ISMXM(x,k), SDMXM(x,k), 
	   DDMXM(x,k), IDMXM(x,k), BIMXM(x,k), SIMXM(x,k), DIMXM(x,k), IiMXM(x,k), iIMXM(x,k), EE);
#endif    
  }
  ...
  for (k = gm->M-1; k >= 0; k --) {
	...
	sc = p7_FLogsum(sc, ((apair == NONE)? ISMXM(xv,k) : -eslINFINITY));
	ISMXM(x,k) = p7_FLogsum(sc, EE + e2hmmerTSC(k,e2HP_IS_EE));	
