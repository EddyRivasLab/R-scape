 ...
 * 
 * SVN $Id: shuffle_main.c 1526 2005-12-13 20:20:13Z eddy $
 */
...
main(int argc, char **argv)
{
  ...
  while (ReadSeq(dbfp, dbfp->format, &seq, &sqinfo))
      ...
      for (i = 0; i < num; i++)
	{
	  ...
	  case DO_IID_SAMELEN:
	    free(shuff);
	    if (do_dna) shuff = RandomSequence(DNA_ALPHABET,   dnafq, 20, sqinfo.len);
	    else        shuff = RandomSequence(AMINO_ALPHABET, aafq,  20, sqinfo.len);
	    break;
