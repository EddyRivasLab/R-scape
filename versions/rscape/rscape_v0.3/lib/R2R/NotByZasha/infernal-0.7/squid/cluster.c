 ...
 * SVN $Id: cluster.c 1526 2005-12-13 20:20:13Z eddy $
 *
...
{                 
  ...
  stack = CreateNstack();
  PushNstack(stack, N);	/* push root on stack */
  docomma = FALSE;
  ...
  while (PopNstack(stack, &code))
      ...
      else if (code < 2*N)      /* we're an interior node */
	{
	  ...
	  PushNstack(stack, code+N);
	  PushNstack(stack, tree[code-N].right);
	  PushNstack(stack, tree[code-N].left);
