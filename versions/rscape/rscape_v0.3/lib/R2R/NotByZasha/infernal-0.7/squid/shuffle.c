 ...
 * 
 * The maximum string length is whatever the machine's "int" can count
 * to; on most modern 32-bit machines, that's ~2 GB. Largest human chromosome
 * is ~246 MB. 
 *  
 ...
 * 
 * SVN $Id: shuffle.c 1526 2005-12-13 20:20:13Z eddy $
 */
...
StrMarkov0(char *s1, char *s2)
{
   ...
   * You can't count directly into p, because of limited
   * numeric range of a float.
   */
  for (x = 0; x < 26; x++) c[x] = 0;
  for (pos = 0; pos < len; pos++)
    c[(int)(toupper((int) s2[pos]) - 'A')] ++;
  for (x = 0; x < 26; x++) p[x] = (float) c[x];
  FNorm(p, 26);
