...
bool /* got it */ ParseR2R_LABEL(const std::string& f1,const CommaSepAbstractFile& f,LabelLine& labelLine)
{
...
void CheckNumColumns(size_t& numColumns,int& numColumnsSetLine,size_t numColumnsHere,int currLine) 
{
	if (numColumns==0) {
		...
		if (numColumnsHere==0) {
			throw SimpleStringException("line %d has 0-length sequence/column-annotation",currLine);
		}
			...
			if (f.GetNumFields()>=2) {
				...
				if (doOneSeq) {
					if (f0=="#=GR") {
						...
						if (name==oneSeqName) {
							if (ParseR2R_LABEL(f2,f,labelLine)) {
				...
				if (f0=="#=GC") {
					...
					CheckNumColumns(numColumns,numColumnsSetLine,strlen(f.GetField(f.GetNumFields()-1)),f.GetLineNum());
					if (f1=="cons") {
						consSeq=f.GetField(f.GetNumFields()-1);
					}
					if (f1=="conss") {
...
//		drawingParams.cleavageIndicatorRadius *= drawingParams.nucShrinkWithCircleNuc;
	...
	if (doOneSeq) {
		for (size_t i=0; i<posInfoVector.size(); i++) {
			...
			if ((i+1)%10==0 && drawingParams.defaultOneseqLabeling) {
				if (!posInfoVector[i].nucTickLabel.empty()) {
					posInfoVector[i].nucTickLabel += " , ";
				}
