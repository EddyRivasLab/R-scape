...
#ifdef YaaleRNA
static int FindRightPartner (const std::string& ss,int first)
{
	int len=(int)(ss.size());
	int last=first+1;
	int count=0;
	...
	if (randomizedReselectionMode) {
		...
		if (isalpha(ch)) {
			...
			if (ancestorNuc<0 || ancestorNuc>=MAXABET) {
				throw SimpleStringException("symbol '%c' in randomized reselection #=GC ANCESTOR (pos %d) is apparently degenerate, which I don't support",ch,p);
			}
...
std::string NormalizeSs (const char *ssRaw,int len)
{
	...
	for (int ssp=0; ssp<len; ssp++) {
		...
		if (!isprint(ssRaw[ssp])) {
		  if (ssRaw[ssp]==13) {
			  throw SimpleStringException("There is a carriage-return character (ASCII character code 13) in the file, which creates problems with this program.  These characters come from files created/edited in Microsoft Windows.  Please remove the character(s) using the Undos.pl script included in this software package (run: perl Undos.pl <file>).  You can read the Wikipedia article at http://en.wikipedia.org/wiki/Newline for more information.");
		  }
		  else {
			  throw SimpleStringException("unprintable character in an SS_cons line.  Character code %d at position %d within the line.  The text in the line is \"%s\" (%s:%d)",(int)(ssRaw[ssp]),ssp,ssRaw,__FILE__,__LINE__);
		  }
		}
		switch (ssRaw[ssp]) {
			case '<':
...
{
	if (ssRaw==NULL) {
		throw SimpleStringException("no #=GC SS_cons line was found in the input alignment");
	}
...
void GSCWeightedConsensus_Calculate(MSA *msa,GSCWeightedConsensus_Output& output,GSCWeightedConsensus_Input& input)
{
	...
	for (int t=0; t<msa->ngf; t++) {
		...
		if (strcmp(msa->gf_tag[t],"USE_THIS_WEIGHT_MAP")==0) {
			...
			if (sep.GetNumFields()!=msa->nseq*2) {
				throw SimpleStringException("in #=GF USE_THIS_WEIGHT_MAP, there were %d items, which should be 2 times the number of sequences in the alignment (and there are %d sequences, twice of which is %d)",
					sep.GetNumFields(),msa->nseq,msa->nseq*2);
			}
			if (sep.GetNumFields()%2 !=0) {
				throw SimpleStringException("in #=GF USE_THIS_WEIGHT_MAP, there were an odd number of items, but it should be a space-separated list of  hitId<space>weight");
			}
			...
			for (int i=0; i<msa->nseq; i++) {
				...
				if (findIter==weightMap.end()) {
					throw SimpleStringException("in #=GF USE_THIS_WEIGHT_MAP, the hitId \"%s\" was used in the alignment, but did not have an associated weight in the USE_THIS_WEIGHT_MAP list",hit.c_str());
				}
	...
	if (randomizedReselectionMode) {
		if (randomizedRelesectionFreqOfAncestorNucleotide==-1) {
			throw SimpleStringException("used RANDOMIZED RESELECTION mode, but you didn't give me the freq of ancestor");
		}
		if (!useManualWeights) {
			throw SimpleStringException("used RANDOMIZED RESELECTION mode, but you didn't use manual sequence weights, which seems suspicious");
		}
		...
		if (randomizedRelectionAncestorSeq.length()==0) {
			throw SimpleStringException("used RANDOMIZED RESELECTION mode, but didn't include #=GC ANCESTOR to specify the ancestor molecule in the randomized region");
		}
	}
	...
	GSCWeightedConsensus_SS(output.numPairsWithCovariation,output.ssCovaryLines,input.nonCanonPairThreshold,msa,"SS_cons",msa->ss_cons,validSeqRegionVector,
		randomizedReselectionMode,randomizedRelectionAncestorSeq,randomizedRelesectionFreqOfAncestorNucleotide,mutualInfoFile);
	if (mutualInfoFile!=NULL) {
	...
	for (int t=0; t<msa->ngc; t++) {
		if (strncmp(msa->gc_tag[t],SS_cons,strlen(SS_cons))==0) {
			GSCWeightedConsensus_SS(output.numPairsWithCovariation,output.ssCovaryLines,input.nonCanonPairThreshold,msa,msa->gc_tag[t],msa->gc[t],validSeqRegionVector,
				randomizedReselectionMode,randomizedRelectionAncestorSeq,randomizedRelesectionFreqOfAncestorNucleotide,NULL);
		}
	}
	for (int i=0; i<msa->nseq; i++) {
...
void GSCWeightedConsensus(GSCWeightedConsensus_Input& input)
{
	...
	if (msa==NULL) {
		throw SimpleStringException("no MSA in input file %s",input.stoFileName);
	}
