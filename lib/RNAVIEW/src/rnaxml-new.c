               ...
               long **multi, long *sugar_syn)
{
    ...
    for (i=1; i<=nchain; i++){
	    ...
	    POSITION_1_N[k]=j;
            chain_res[k]=i;  
                /*       
            printf("chain_idx chain_res %4ld %4ld %4ld %4ld  %c: %4ld %4ld %4ld  %c\n",
...
/*
                printf("isolated bases %4ld %4ld %4ld %4ld %4ld\n",
    ...
    for(i=1; i<=xml_nh ; i++){    
        ...
        if(chain1>0 && chain2>0){ /* interaction */
            ...
            if(k>0){
                ...
                fprintf(xml,"%s</helix>\n",tag5);
/*
                printf("helix num %ld %ld ;%ld %ld %ld ; %ld %ld \n",
