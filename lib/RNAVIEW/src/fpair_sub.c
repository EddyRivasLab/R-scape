...
long number_of_atoms(char *pdbfile, long *ret_natom_tot)
/* number of atom records in a PDB file */
{
  ...
  while (fgets(str, sizeof str, fp) != NULL) {
    nlen = upperstr(str);
    if (!strncmp(str, "ATOM", 4) || !strncmp(str, "HETATM", 6)) natom_tot ++;
 ...
 */
{
  ...
  while (fgets(str, sizeof str, fp) != NULL) {
	...
	exit(1);
      ...
      temp[25] = '\0';
      if (sscanf(temp,"%8lf%8lf%8lf", &xyz[n][1], &xyz[n][2], &xyz[n][3]) != 3)
...
/* number of residues, and starting-ending indexes for each */
{
  ...
  *num_residue = n;
  ...
  n = 0;
  for (i = 1; i <= num;          i++) if (temp[i]) seidx[++n][2] = i;
  for (n = 2; n <= *num_residue; n++) seidx[n][1] = seidx[n - 1][2] + 1;
  seidx[1][1] = 1;
 ...
 */
{
    ...
    if (N1 && C2 && C6) {
        for (i = 1; i <= 3; i++) temp[i] = xyz[N1][i] - xyz[C2][i];
        d1 = veclen(temp);
	
        for (i = 1; i <= 3; i++) temp[i] = xyz[N1][i] - xyz[C6][i];
        d2 = veclen(temp);
	
        for (i = 1; i <= 3; i++) temp[i] = xyz[C2][i] - xyz[C6][i];
        d3 = veclen(temp);
    ...
    if (CA && C) {
        for (i = 1; i <= 3; i++) temp[i] = xyz[CA][i] - xyz[C][i];
        if (veclen(temp) <= dcrt)
            id = -1;
        return id;
    }
