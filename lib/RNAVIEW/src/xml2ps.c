...
static void get_sequence(char *inpfile, char *resname, long *author_seq, long *nres,
			 char **RESNAME, long **AUTH_SEQ, long *MNUM, long *AUTH_SEQ_IDX);
static int get_mol_num(char *inpfile);
static void new_position(long id2, long position, long *position_new);
static void read_sugar_syn(char *inpfile, long **sugar_syn);
static void get_sugar_syn(FILE *inp, char *value_ch);
static void get_chain_broken(long nres, double **a, double **b, long *chain_broken);
...
/*
            printf("SUGAR_SYN %ld %ld %ld %ld %ld\n", i, k,n, sugar_syn[n], SUGAR_SYN[i][k]);
*/
            k++;
            n++;
        }
...
/* fprintf(psfile,"%s : %ld  ",item, position);*/
                                                }
                                                if(!strcmp(item,"BASE-ID-3P")){
                                                    do{
                                                        element_in_bracket(inp, item, &size, lett, &key);
...
/* fprintf(psfile,"%s : %ld  ",item, position);*/
                                                ...
                                                if(!strcmp(item,"/HELIX")) {
						  /*printf(" INHELIX %s : molID= %4d %4d   %ld %ld  %ld  %ld \n",
        ...
        if(strcmp(item,"MOLECULE")==0){
            ...
            do{
                ...
                if(!strcmp(item,"SEQUENCE")){    /* for identity */
                   ...
                   /*                        
                  printf("parsing sequnece!  %s : %s %ld\n", item,resname, *nres);
                    */            
