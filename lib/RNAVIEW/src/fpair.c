  ...
  for (i = 1; i < num_residue; i++) {
	...
	*/         
	...
	if (nnh == 1){ /*single base-to-base H bond */
	  ...
	  if (status != eslOK)  {
	    printf("wrong BPTYPE %s\n", type);
	    ESL_XFAIL(eslFAIL, errbuf, "wrong BPTYPE %s", type);
	  }
...
/* get the protein and nucleic acid interactions */
{
    ...
    long     n = 0;
    long     m;
    long     interact, prot_atom = 0, prot_atom_tot;
    ...
    prot_atom = number_of_atoms("protein.pdb", &prot_atom_tot);
