...
static int map_alignment(const char *msafile, const P7_HMM *hmm, ESL_SQ ***ret_sq, P7_TRACE ***ret_tr, int *ret_ntot, ESL_MSA **ret_msa);
...
static ESL_OPTIONS options[] = {
  ...
  { "--xiz",       eslARG_REAL,    "0.01",     NULL, "x>=0", NULL,    NULL,  NULL, "xiI at time zero",                                                  0 },
  { "--rz",        eslARG_REAL,    "0.01",     NULL, "x>=0", NULL,    NULL,  NULL, "r_0 at time zero",                                                  0 },
  { "--xiIinfty",  eslARG_REAL,    "0.99",     NULL, "x>=0", NULL,    NULL,  NULL, "xiI at time infinity",                                              0 },
  { "--vanilla",   eslARG_NONE,     FALSE,     NULL, NULL,   NULL,    NULL,  NULL, "vanilla hmmalign",                                                  0 },
...
main(int argc, char **argv)
{
  ...
  ESL_MSA      *mapmsa  = NULL;	/* multiple alignment the HMM was build from    */
  ...
  /* added options */
  xiz     = esl_opt_GetReal(go, "--xiz");
  rz      = esl_opt_GetReal(go, "--rz");
  xiIinf  = esl_opt_GetReal(go, "--xiIinfty");
  vanilla = esl_opt_GetBoolean(go, "--vanilla");
  ...
  if ( (mapfile = esl_opt_GetString(go, "--mapali")) != NULL)
    {
      map_alignment(mapfile, hmm, &sq, &tr, &mapseq, &mapmsa);
    }
  totseq = mapseq;
  ...
  p7_tracealign_Seqs(sq, tr, totseq, om->M, msaopts, hmm, &msa);

  if (!vanilla) {
    if (p7_EvoMSA(r, sq, tr, hmm, bg, NULL, msaopts, mapmsa, msa, xiz, rz, xiIinf, tol, errbuf, verbose) != eslOK)  esl_fatal("%s\nAlignment optimization failed", errbuf);
  }
...
static int
map_alignment(const char *msafile, const P7_HMM *hmm, ESL_SQ ***ret_sq, P7_TRACE ***ret_tr, int *ret_ntot, ESL_MSA **ret_msa)
{
  ESL_SQ       **sq        = NULL;
  P7_TRACE     **tr        = NULL;
 ...
 * @LICENSE@
 *
 * SVN $URL: https://svn.janelia.org/eddylab/eddys/src/hmmer/trunk/src/hmmalign.c $
