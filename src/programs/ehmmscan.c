...
serial_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
  ...
  for (i = 0; i < infocnt; ++i)
    {
      info[i].emR     = ratematrix_emrate_Clone(emR);
  ...
  for (i = 0; i < infocnt; ++i) {
    ...
    ratematrix_emrate_Destroy(info[i].emR, 1);
  }
	  ...
	  while (count > 0 && (hstatus = p7_oprofile_ReadMSV(hfp, &abc, &om)) == eslOK)
	    {
	      ...
	      if (p7_EvoPipeline(pli, emR, /*R=*/NULL, /*hmm=*/hmm, /*gm=*/gm, om, bg, qsq, th, noevo, fixtime, FALSE) != eslOK) {
  ...
  while ((status = p7_oprofile_ReadMSV(hfp, &abc, &om)) == eslOK)
    {
      ...
      if (p7_EvoPipeline(info->pli, info->emR, /*R=*/NULL, /*hmm=*/NULL, /*gm=*/NULL, om, info->bg, info->qsq, info->th, info->noevo, info->fixtime, FALSE) != eslOK) {
	printf("error in p7_EvoPipeline\n");
	exit(1);
      }
 ...
 * @LICENSE@
 *
 * SVN $Id: hmmscan.c 4349 2012-12-19 22:07:54Z eddys $
 * SVN $URL: https://svn.janelia.org/eddylab/eddys/src/hmmer/trunk/src/programs/hmmscan.c $
 *****************************************************************/
