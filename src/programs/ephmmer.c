...
/* Control of output */
  ...
  { "--hmmout",     eslARG_OUTFILE,      NULL, NULL, NULL,      NULL,  NULL,  NULL,              "save singleseq hmm to file <s>",                               2 },
...
serial_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
  ...
  FILE            *hmmfp    = NULL;		  /* output hmm per-seq (--hmmout)                    */
  ...
  if (esl_opt_IsOn(go, "--hmmout"))    { if ((hmmfp    = fopen(esl_opt_GetString(go, "--hmmout"),    "w")) == NULL)  p7_Fail("Failed to open tabular per-seq output file %s for writing\n", esl_opt_GetString(go, "--hmmfp")); }
 ...
 /* other options */
  ...
  while ((qstatus = esl_sqio_Read(qfp, qsq)) == eslOK)
      ...
      if (nquery > 1)
	{
      ...
      /* Build the model */
      p7_SingleBuilder(bld, qsq, info[0].bg, &hmm, NULL, &gm, &om); /* bypass HMM - only need model */  
      if (hmmfp) {
      ...
      for (i = 1; i < infocnt; ++i)
	{
      ...
      if (tblfp)     p7_etophits_TabularTargets(tblfp,    hmm->name, hmm->acc, info->th, info->pli, (nquery == 1));
      if (domtblfp)  p7_etophits_TabularDomains(domtblfp, hmm->name, hmm->acc, info->th, info->pli, (nquery == 1));
...
mpi_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
  ...
  if (esl_opt_IsOn(go, "--hmmout")    && (hmmfp    = fopen(esl_opt_GetString(go, "--hmmout"),    "w")) == NULL)
    mpi_failure("Failed to open hmm per-seq output file %s for writing\n", esl_opt_GetString(go, "--hmmfp"));
  ...
  while ((qstatus = esl_sqio_Read(qfp, qsq)) == eslOK)
    {
      ...
      /* Build the model */
      p7_SingleBuilder(bld, qsq, bg, &hmm, NULL, NULL, &om); /* bypass HMM - only need model */
      if (hmmfp) {
	if ((status = p7_hmmfile_WriteASCII(hmmfp, -1, hmm)) != eslOK) ESL_FAIL(status, errbuf, "HMM save failed");
      }
 ...
 * @LICENSE@
 *
 * SVN $Id: phmmer.c 4426 2013-04-18 20:19:09Z wheelert $
 * SVN $URL: https://svn.janelia.org/eddylab/eddys/src/hmmer/trunk/src/programs/phmmer.c $
 *****************************************************************/
