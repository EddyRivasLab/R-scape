...
serial_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
  ...
  while (hstatus == eslOK) 
    {
       ...
       */
      if (!esl_opt_IsOn(go, "--noevo") && 
	  p7_RateCalculate(statfp, hmm, info[0].bg, emR, NULL, &R, evomodel, betainf, info[0].fixtime, 0.001, errbuf, FALSE) != eslOK)  
	esl_fatal("%s", errbuf);
   ...
   */
  if (tblfp)    p7_tophits_TabularTail(tblfp,    "ehmmsearch", p7_SEARCH_SEQS, cfg->hmmfile, cfg->dbfile, go);
  if (domtblfp) p7_tophits_TabularTail(domtblfp, "ehmmsearch", p7_SEARCH_SEQS, cfg->hmmfile, cfg->dbfile, go);
...
mpi_worker(ESL_GETOPTS *go, struct cfg_s *cfg)
{
  ...
  while (hstatus == eslOK) 
	...
	ratematrix_emrate_Set(esl_opt_GetString(go, "--rmx"), (const double *)bg->f, emR, tol, errbuf, FALSE);
      ...
      th  = p7_tophits_Create(p7_TOPHITS_DEFAULT_INIT_ALLOC); 
      xpli = p7_pipeline_Create(go, om->M, 100, FALSE, p7_SEARCH_SEQS); /* L_hint = 100 is just a dummy for now */
      p7_pipeline_NewModel(pli, om, bg);
	  ...
	  while (count > 0 && (sstatus = esl_sqio_Read(dbfp, dbsq)) == eslOK)
	    {
	      ...
	      if (p7_EvoPipeline(pli, R, hmm, gm, om, bg, dbsq, th, noevo, fixtime, FALSE) != eslOK) {
