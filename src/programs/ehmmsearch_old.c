...
static ESL_OPTIONS options[] = {
  ...
  { "--F2",         eslARG_REAL,  "1e-2", NULL, NULL,       NULL,  NULL, "--max",          "Stage 2 (Vit) threshold: promote hits w/ P <= F2",             7 },
  { "--F3",         eslARG_REAL,  "1e-5", NULL, NULL,       NULL,  NULL, "--max",          "Stage 3 (Fwd) threshold: promote hits w/ P <= F3",             7 },
  { "--F4",         eslARG_REAL,  "1e-6", NULL, NULL,       NULL,  NULL, "--max",          "Stage 4 (time-Fwd) threshold: promote hits w/ P <= F4",        7 },
  { "--nobias",     eslARG_NONE,   NULL,  NULL, NULL,       NULL,  NULL, "--max",          "turn off composition bias filter",                             7 },
/* evolutionary options */
  { "--noevo",      eslARG_NONE,   FALSE, NULL, NULL,       NULL,  NULL,  NULL,            "do not evolve",                                                0 },
  { "-R",           eslARG_NONE,   FALSE, NULL, NULL,       NULL,  NULL,  NULL,            "evolve emissions using a rate matrix",                         0 },
  { "--rmx",        eslARG_STRING, "BLOSUM62", NULL, NULL,  NULL,  "-R",  NULL,            "substitution rate matrix choice (of some built-in matrices)",  0 },
  ...
  { "--etaz",       eslARG_REAL,  "0.40", NULL, "x>=0",     NULL,  NULL,  NULL,            "etaz = eta at time zero",                                      0 },
  { "--hmmifile",   eslARG_INFILE, NULL,  NULL, NULL,       NULL,  NULL,  NULL,            "read hmm at infinity from <f>",                                0 },
 /* Other options */
  ...
  { "--tol",        eslARG_REAL,"0.0001", NULL, NULL,       NULL,  NULL,  NULL,            "tolerance",                                                   12 },
#ifdef HMMER_THREADS 
...
output_header(FILE *ofp, const ESL_GETOPTS *go, char *hmmfile, char *seqfile)
{
  ...
  if (esl_opt_IsUsed(go, "--F4")           && fprintf(ofp, "# time-Fwd filter P threshold:  <= %g\n",             esl_opt_GetReal(go, "--F4"))           < 0) ESL_EXCEPTION_SYS(eslEWRITE, "write failed");
  ...
  if (esl_opt_IsUsed(go, "--evomodel")     && fprintf(ofp, "# evomodel:                        %s\n",             esl_opt_GetString(go, "--evomodel"))   < 0) ESL_EXCEPTION_SYS(eslEWRITE, "write failed");
  if (esl_opt_IsUsed(go, "--betainf")      && fprintf(ofp, "# beta at time infinity set to:    %.0f\n",           esl_opt_GetReal(go, "--betainf"))      < 0) ESL_EXCEPTION_SYS(eslEWRITE, "write failed");
  if (esl_opt_IsUsed(go, "--etainf")       && fprintf(ofp, "# eta at time infinity set to:     %.0f\n",           esl_opt_GetReal(go, "--etainf"))       < 0) ESL_EXCEPTION_SYS(eslEWRITE, "write failed");
  if (esl_opt_IsUsed(go, "--etaz")         && fprintf(ofp, "# eta at time zero set to:         %.0f\n",           esl_opt_GetReal(go, "--etaz"))         < 0) ESL_EXCEPTION_SYS(eslEWRITE, "write failed");
  if (esl_opt_IsUsed(go, "--tol")          && fprintf(ofp, "# tolerance set to:                %.0f\n",           esl_opt_GetReal(go, "--tol"))          < 0) ESL_EXCEPTION_SYS(eslEWRITE, "write failed");
...
serial_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
  ...
  P7_HMMFILE      *hifp     = NULL;           /* open input HMMinf file                             */
  P7_HMM          *hmminf   = NULL;           /* one HMMinf query                                   */
  EMRATE          *emR      = NULL;
  EVOM             evomodel;
  float            betainf;
  float            etainf;
  ...
  if (esl_opt_IsUsed(go, "-R")) {
    ...
    ratematrix_emrate_Set(esl_opt_GetString(go, "--rmx"), NULL, (const double *)(info[0].bg->f), emR, tol, errbuf, FALSE);
  }

  if (esl_opt_GetString(go, "--hmmifile") != NULL) 
    { 
      cfg->hmmifile = esl_opt_GetString(go, "--hmmifile");
      status = p7_hmmfile_OpenE(cfg->hmmifile, NULL, &hifp, errbuf);
      if      (status == eslENOTFOUND) p7_Fail("File existence/permissions problem in trying to open HMMi file %s.\n%s\n", cfg->hmmifile, errbuf);
      else if (status == eslEFORMAT)   p7_Fail("File format problem in trying to open HMMi file %s.\n%s\n",                cfg->hmmifile, errbuf);
      else if (status != eslOK)        p7_Fail("Unexpected error %d in opening HMM ifile %s.\n%s\n",               status, cfg->hmmifile, errbuf);  
      
     hstatus = p7_hmmfile_Read(hifp, &abc, &hmminf);
      if (hstatus != eslOK) p7_Fail("Couldn't read file", cfg->hmmifile);
    }
  ...
  while (hstatus == eslOK) 
      ...
      if (nquery > 1)
	{
      ...
      if (p7_RateCalculate(hmm, info[0].bg, emR, NULL, &R, evomodel, betainf, 0.001, errbuf, FALSE) != eslOK)  esl_fatal("%s", errbuf);
      ...
      for (i = 0; i < infocnt; ++i)
	{
	  ...
	  info[i].gm  = p7_profile_Clone(gm);
 	  //info[i].om  = p7_oprofile_Clone(om); produces memory problem wehn using p7_EvoPipeline(_
	  info[i].om = p7_oprofile_Create(hmm->M, abc);
	  p7_oprofile_Convert(gm, info[i].om);
	...
	esl_msa_Destroy(msa);
      ...
      p7_hmm_Destroy(info->hmm);
      p7_RateDestroy(info->R);
      p7_profile_Destroy(info->gm);
      p7_oprofile_Destroy(info->om);
      p7_oprofile_Destroy(om);
      p7_profile_Destroy(gm);
      p7_hmm_Destroy(hmm);
      if (hmminf) p7_hmm_Destroy(hmminf);
      p7_RateDestroy(R);
...
mpi_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
  ...
  while (hstatus == eslOK) 
	...
	ratematrix_emrate_Set(esl_opt_GetString(go, "--rmx"), (const double *)bg->f, emR, tol, errbuf, FALSE);
      }
      if (esl_opt_GetString(go, "--hmmifile") != NULL) 
	{ 
	  cfg->hmmifile = esl_opt_GetString(go, "--hmmifile");
     ...
     /* Calculate the hmm rate */
      if (p7_RateCalculate(hmm, bg, emR, NULL, &R, evomodel, betainf, 0.001, errbuf, FALSE) != eslOK)  esl_fatal("%s", errbuf);
...
mpi_worker(ESL_GETOPTS *go, struct cfg_s *cfg)
{
  ...
  if (esl_opt_IsUsed(go, "-R")) {
    cfg->hmmifile = esl_opt_GetString(go, "--hmmifile");
    emR = ratematrix_emrate_Create(bg->abc);
    ratematrix_emrate_Set(esl_opt_GetString(go, "--rmx"), bg->f, emR, tol, errbuf, FALSE);
  }
  if (esl_opt_GetString(go, "--hmmifile") != NULL) 
    { 
      status = p7_hmmfile_OpenE(cfg->hmmifile, NULL, &hifp, errbuf);
  ...
  while (hstatus == eslOK) 
    {
      ...
      /* Calculate the hmm rate */
      if (p7_RateCalculate(hmm, bg, emR, NULL, &R, evomodel, betainf, 0.001, errbuf, FALSE) != eslOK) {
	printf("%s\n", errbuf);
	exit(1);
      }
	  ...
	  while (count > 0 && (sstatus = esl_sqio_Read(dbfp, dbsq)) == eslOK)
	    {
	      ...
	      if (p7_EvoPipeline(pli, R, hmm, gm, om, bg, dbsq, th, noevo, FALSE) != eslOK) {
		printf("error in evopipeline\n");
		exit(1);
	      }
  ...
  while ((sstatus = esl_sqio_Read(dbfp, dbsq)) == eslOK)
    {
      ...
      if (p7_EvoPipeline(info->pli, info->R, info->hmm, info->gm, info->om, info->bg, dbsq, info->th, info->noevo, FALSE)) {
	printf("error in evopipeline for %s\n", dbsq->name);
	exit(1);
      }
  ...
  while (sstatus == eslOK)
    {
      ...
      sstatus = esl_sqio_ReadBlock(dbfp, block, -1, FALSE, FALSE);
      if (sstatus == eslEOF)
	{
 ...
 * @LICENSE@
 * 
 * SVN $URL: https://svn.janelia.org/eddylab/eddys/src/hmmer/trunk/src/programs/hmmsearch.c $
