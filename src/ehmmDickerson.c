/* ehmmDickerson -- given an msa (or a collection of sequences and a tree)
 *                  estimate the pairwise rates of molecular evolution
 *                  using a evolutionary hmm
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static ESL_OPTIONS options[] = {
  ...
  { "--group",      eslARG_INT,                                 "10",  NULL, "n>0",    NULL,    NULL,  NULL,               "the approximate number of groups that the species should be grouped into", 0 },
  { "--noopt",      eslARG_NONE,                               FALSE,  NULL,  NULL,    NULL,    NULL,  NULL,               "TRUE for no optimization of parameters",                                   0 }, 
   /* options for msa */
...
static char banner[] = "calculate pairwise rates of evolution using an evolutionary  profile hmm";

static int ehmmDickerson_onecluster(FILE *outfp, char *gnuplot, char *spfile, char *distfile, char *histofile, char *ratefile, char *binratefile,
				    ESL_RANDOMNESS *r, ESL_MSA *msa, ESL_TREE *T, ORTHO *ortho, P7_PIPELINE *pli, P7_BG *bg, 
				    float evalcutoff, P7_RATE *R, int noopt, int do_viterbi, float bintime, float tlinear, int full, float tol, int view, char *errbuf, int verbose);
...
static int outfile_header(char *acc, int full, char **ret_outheader);
...
process_commandline(int argc, char **argv, ESL_GETOPTS **ret_go, struct cfg_s *ret_cfg)
{
  ...
  cfg.noopt      = esl_opt_GetBoolean(go, "--noopt");
...
main(int argc, char **argv)
{
  ...
  /* Initializations */
  cfg.pli = p7_pipeline_Create(go, hmm->M, 100, FALSE, p7_SEARCH_SEQS);
  cfg.bg = p7_bg_Create(cfg.abc);
  ...
  if (!esl_opt_IsOn(go, "--noevo") && 
      p7_RateCalculate(statfp, hmm, cfg.bg, emR, NULL, &cfg.R, cfg.evomodel, cfg.betainf, cfg.fixtime, 0.001, cfg.errbuf, FALSE) != eslOK)  
   ...
   output_header(cfg.outfp, go);   
	...
	if (ehmmDickerson_onecluster(cfg.outfp, cfg.gnuplot, famspfile, famdistfile, histofile, ratefile, binratefile, cfg.r, msa, T, ortho, cfg.pli, cfg.bg, 
				     cfg.evalcutoff, cfg.R, cfg.noopt, cfg.do_viterbi, cfg.bintime, cfg.tlinear, cfg.full, cfg.tol, cfg.view, cfg.errbuf, cfg.verbose)
	...
	for (c = 0; c < nc; c ++) {
      ...
      if (plot_gplot_Histogram(cfg.gnuplot, histops, 1, &histofile, "MYA", cfg.errbuf) == eslOK) { 
	if (cfg.view) plot_file_Display(histops);
      ...
      switch(cfg.plottype) {
      case SERIAL:
	esl_sprintf(&rateps,    "%s.serial.ps", ratefile);
	esl_sprintf(&binrateps, "%s.serial.ps", binratefile);
	remove(rateps);
	remove(binrateps);
	if (plot_gplot_SerialRatesWithLinearRegression(cfg.gnuplot, rateps, 1, &ratefile, cfg.evalcutoff, cfg.xlabel, cfg.ylabel, cfg.errbuf) == eslOK) { 
	  if (cfg.view) plot_file_Display(rateps);
	} else esl_fatal("%s\nbad rates", cfg.errbuf);
	if (plot_gplot_SerialBinnedRatesWithLinearRegression(cfg.gnuplot, binrateps, 1, &binratefile, cfg.xlabel, cfg.ylabel, cfg.errbuf) == eslOK) { 
	  if (cfg.view) plot_file_Display(binrateps);
	} else esl_fatal("%s\nbad binned rates", cfg.errbuf);
	break;
      case TOGETHER:
	esl_sprintf(&rateps,    "%s.together.ps", ratefile);
	esl_sprintf(&binrateps, "%s.together.ps", binratefile);
	remove(rateps);
	remove(binrateps);
	if (plot_gplot_TogetherRatesWithLinearRegression(cfg.gnuplot, rateps, 1, &ratefile, cfg.evalcutoff, cfg.xlabel, cfg.ylabel, cfg.errbuf) == eslOK) { 
	  if (cfg.view) plot_file_Display(rateps);
	} else esl_fatal("%s\nbad rates", cfg.errbuf);
	if (plot_gplot_TogetherBinnedRatesWithLinearRegression(cfg.gnuplot, binrateps, 1, &binratefile, cfg.xlabel, cfg.ylabel, cfg.errbuf) == eslOK) { 
	  if (cfg.view) plot_file_Display(binrateps);
	} else esl_fatal("%s\nbad binned rates", cfg.errbuf);
	break;
      case JOINT:
	esl_sprintf(&rateps,    "%s.joint.ps", ratefile);
	esl_sprintf(&binrateps, "%s.joint.ps", binratefile);
	remove(rateps);
	remove(binrateps);
	...
	free(rateps);    rateps = NULL;
	free(binrateps); binrateps = NULL;
	...
	break;
      ...
      free(histops); histops = NULL;
      if (rateps)free(rateps); rateps = NULL;
      if (binrateps) free(binrateps); binrateps = NULL;
...
static int
ehmmDickerson_onecluster(FILE *outfp, char *gnuplot, char *spfile, char *distfile, char *histofile, char *ratefile, char *binratefile,
			 ESL_RANDOMNESS *r, ESL_MSA *msa, ESL_TREE *T, ORTHO *ortho, P7_PIPELINE *pli, P7_BG *bg,  
			 float evalcutoff, P7_RATE *R, int noopt, int do_viterbi, float bintime, float tlinear, int full, float tol, int view, char *errbuf, int verbose)
{
  ESL_MSA         *omsa = NULL;
  ...
  if (ehmmBranchInference(outfp, gnuplot, distfile, histofile, ratefile, binratefile, 
			  r, omsa, oT, SPE, pli, bg, evalcutoff, R, noopt, do_viterbi, bintime, tlinear, full, tol, view, errbuf, verbose) != eslOK) 
    { printf("%s\n", errbuf); status = eslFAIL; goto ERROR; }
...
static int
outfile_header(char *acc, int full, char **ret_outheader)
{      
  char *outheader = NULL;
  char *st;
  char *name;
