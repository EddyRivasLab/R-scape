...
int
COCOCYK_BGR(ESL_RANDOMNESS *r, struct mutual_s *mi, int **ret_ct, SCVAL *ret_sc, int minloop, int maxFP, double target_eval, char *errbuf, int verbose) 
{
  ...
  for (covthresh = covmax; covthresh > ESL_MAX(0,covmin-covinc); covthresh -= covinc) {
    /* Fill the cyk matrix */
    if ((status = COCOCYK_BGR_Fill(mi, cyk, ret_sc, minloop, covthresh, errbuf, verbose)) != eslOK) goto ERROR;    
    /* Report a traceback */
    if ((status = COCOCYK_BGR_Traceback(r, mi, cyk, &ct, minloop, covthresh, errbuf, verbose))  != eslOK) goto ERROR;
...
int
COCOCYK_BGR_Fill(struct mutual_s *mi, GMX *cyk, SCVAL *ret_sc, int minloop, double covthresh, char *errbuf, int verbose) 
{
  SCVAL  sc;
  int    L = mi->alen;
  int    j, d;
...
int
COCOCYK_BGR_Traceback(ESL_RANDOMNESS *rng, struct mutual_s *mi, GMX *cyk, int **ret_ct, int minloop, double covthresh, char *errbuf, int verbose) 
{
