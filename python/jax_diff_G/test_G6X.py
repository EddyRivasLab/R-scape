...
def test_G6X_Inside_JAX(n: int, single_seq: int, verbose: int, min_hairpin: int=0):
    ...
    p_inside_jax_scaled   = g6x_inside_jax_scaled(psq, psq2, t0, t1, t2, pe_single, pe_pair)                                 # scaled jax
    logp_inside_std_scaled   = G6X_Inside_std_scaled(psq, psq2, t0, t1, t2, pe_single, pe_pair, scale, verbose, K, min_hairpin) # scaled standaar
    logp_inside_jax_scaled = np.log(p_inside_jax_scaled) - n * np.log(scale)

    g6x_inside_jax  = G6X_Inside_JAX(verbose, K, min_hairpin)
    logp_inside_jax = g6x_inside_jax(log_psq, log_psq2, log_t0, log_t1, log_t2, e_single, e_pair)                          # scaled jax
    logp_inside_std = G6X_Inside_std(log_psq, log_psq2, log_t0, log_t1, log_t2, e_single, e_pair, verbose, K, min_hairpin) # logspace standard
    ...
    # jax_scaled
    g6x_inside_grad_scaled = value_and_grad(g6x_inside_jax_scaled, argnums=(2,3,4,5,6))
    value_scaled, grads_scaled = g6x_inside_grad_scaled(psq, psq2, t0, t1, t2, pe_single, pe_pair)
    print("p_inside_g6x:", value_scaled)
    print("p_inside_g6x_grads:", grads_scaled)

     # gradients wrt parameters
    # jax_log
    g6x_inside_grad = value_and_grad(g6x_inside_jax, argnums=(2,3,4,5,6))
    value_log, grads_log = g6x_inside_grad(log_psq, log_psq2, log_t0, log_t1, log_t2, e_single, e_pair)
