...
import  grammars.g6x.g6x_params as g6x_params
...
def G6X_inside_calculate(args, rundir, K, mask, psq, psq2, log_psq, log_psq2, psq_name, verbose):
    ...
    scale       = K
    if scaled: print("G6X params (scaled)")
    else:      print("G6X params (logsumexp)")

    # the parameters
    if   param_tornado: log_t0, t0, log_t1, t1, log_t2, t2, e_single, pe_single, e_pair, pe_pair = g6x_params.G6X_param_tornado(verbose)
    elif param_naive:   log_t0, t0, log_t1, t1, log_t2, t2, e_single, pe_single, e_pair, pe_pair = g6x_params.G6X_param_naive(verbose)
    ...
    for i in range(len(psq)):
        ...
        #p_inside_jax_scaled = g6x_inside_jax_scaled(mask_i, psq_i, psq2_i, t0, t1, t2, pe_single, pe_pair)                                    # scaled jax
        #logp_inside_std_scaled = G6X_Inside_std_scaled(mask_i, psq_i, psq2_i, t0, t1, t2, pe_single, pe_pair, scale, verbose, K, min_hairpin) # scaled standard
        #logp_inside_jax_scaled = np.log(p_inside_jax_scaled) - n * np.log(scale)
        
        logp_inside_jax = g6x_inside_jax(mask_i, log_psq_i, log_psq2_i, log_t0, log_t1, log_t2, e_single, e_pair)                          # logspace jax
        #logp_inside_std = G6X_Inside_std(mask_i, log_psq_i, log_psq2_i, log_t0, log_t1, log_t2, e_single, e_pair, verbose, K, min_hairpin) # logspace standard
...
def main(args):
    ...
    # calculate inside
    G6X_inside_calculate(args, rundir, K, mask, psq, psq2, log_psq, log_psq2, psq_name, verbose)
