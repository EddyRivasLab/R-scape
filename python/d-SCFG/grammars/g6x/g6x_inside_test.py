...
from grammars.g6x.g6x_params import (
    G6X_param_tornado,
    G6X_param_uniform,
)


def test_G6X_Inside_JAX(n: int, single_seq: int, verbose: int, min_hairpin: int=0):
    K = 4
    scale = 1

    log_t0, t0, log_t1, t1, log_t2, t2, e_single, pe_single, e_pair, pe_pair = G6X_param_tornado(verbose)
    ...
    g6x_inside_jax_scaled = G6X_Inside_JAX_scaled(scale, verbose, K, min_hairpin)
    p_inside_jax_scaled   = g6x_inside_jax_scaled(mask, psq, psq2, t0, t1, t2, pe_single, pe_pair)                                 # scaled jax
    logp_inside_std_scaled   = G6X_Inside_std_scaled(mask, psq, psq2, t0, t1, t2, pe_single, pe_pair, scale, verbose, K, min_hairpin) # scaled standaar
    logp_inside_jax_scaled = np.log(p_inside_jax_scaled) - n * np.log(scale)

    g6x_inside_jax  = G6X_Inside_JAX(verbose, K, min_hairpin)
    logp_inside_jax = g6x_inside_jax(mask, log_psq, log_psq2, log_t0, log_t1, log_t2, e_single, e_pair)                          # scaled jax
    logp_inside_std = G6X_Inside_std(mask, log_psq, log_psq2, log_t0, log_t1, log_t2, e_single, e_pair, verbose, K, min_hairpin) # logspace standard
    ...
    # jax_scaled
    g6x_inside_grad_scaled = value_and_grad(g6x_inside_jax_scaled, argnums=(3,4,5,6,7))
    value_scaled, grads_scaled = g6x_inside_grad_scaled(mask, psq, psq2, t0, t1, t2, pe_single, pe_pair)
    print("p_inside_g6x:", value_scaled)
    print("p_inside_g6x_grads:", grads_scaled)

     # gradients wrt parameters
    # jax_log
    g6x_inside_grad = value_and_grad(g6x_inside_jax, argnums=(3,4,5,6,7))
    value_log, grads_log = g6x_inside_grad(mask, log_psq, log_psq2, log_t0, log_t1, log_t2, e_single, e_pair)
    print("logp_inside_g6x:", value_log)
    print("logp_inside_g6x_grads:", grads_log)
...
def main():
    ...
    test_G6X_Inside_JAX(n, single_seq, verbose, min_hairpin=0)
    test_G6X_Inside_JAX(n, single_seq, verbose, min_hairpin=3)
