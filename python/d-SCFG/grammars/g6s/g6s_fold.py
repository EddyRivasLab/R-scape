...
from  grammars.g6s.g6s_inside import (
    G6S_Inside_JAX,
    G6S_Inside_std,
    G6S_Inside_JAX_scaled,
    G6S_Inside_std_scaled,
)
 
import  grammars.g6s.g6s_params as g6s_params
...
def G6S_inside_calculate(args, rundir, K, mask, psq, psq2, log_psq, log_psq2, psq_name, verbose):
    ...
    scale       = K
    if scaled: print("G6S params (scaled)")
    else:      print("G6S params (logsumexp)")

    # the parameters
    if   param_tornado: log_t0, t0, log_t1, t1, log_t2, t2, e_single, pe_single, e_pair, pe_pair = g6s_params.G6S_param_tornado(verbose)
    elif param_naive:   log_t0, t0, log_t1, t1, log_t2, t2, e_single, pe_single, e_pair, pe_pair = g6s_params.G6S_param_naive(verbose)
    else:               log_t0, t0, log_t1, t1, log_t2, t2, e_single, pe_single, e_pair, pe_pair = g6s_params.G6S_param_uniform(K, verbose)

    g6s_inside_jax_scaled = G6S_Inside_JAX_scaled(scale, verbose, K, min_hairpin)
    g6s_inside_jax        = G6S_Inside_JAX(              verbose, K, min_hairpin)
    ...
    for i in range(len(psq)):
        ...
        #p_inside_jax_scaled = g6s_inside_jax_scaled(mask_i, psq_i, psq2_i, t0, t1, t2, pe_single, pe_pair)                                    # scaled jax
        #logp_inside_std_scaled = G6S_Inside_std_scaled(mask_i, psq_i, psq2_i, t0, t1, t2, pe_single, pe_pair, scale, verbose, K, min_hairpin) # scaled standard
        #logp_inside_jax_scaled = np.log(p_inside_jax_scaled) - n * np.log(scale)
        
        logp_inside_jax = g6s_inside_jax(mask_i, log_psq_i, log_psq2_i, log_t0, log_t1, log_t2, e_single, e_pair)                          # logspace jax
        #logp_inside_std = G6S_Inside_std(mask_i, log_psq_i, log_psq2_i, log_t0, log_t1, log_t2, e_single, e_pair, verbose, K, min_hairpin) # logspace standard
...
def main(args):
    ...
    mask, psq, log_psq, psq_name = seqio.read_fasta(filename, shuffle, bymin)
    ...
    # calculate inside
    G6S_inside_calculate(args, rundir, K, mask, psq, psq2, log_psq, log_psq2, psq_name, verbose)
