...
def get_argparse():
    ...
    parser.add_argument('--acc_ymin', type=float, default = -4.0)
    parser.add_argument('--acc_ymax', type=float, default = 80.0)
...
def optimize_param_G6(args, run_dir, K, mask, psq, log_psq, grad_accum, acc_ymin, acc_ymax, verbose):
    ...
    if os.path.exists(test_file):
        ...
        test_outdir =  Path(str(run_dir)+"/"+test_name+"."+fold_method)
        ...
        if os.path.exists(param_file_ML):
            g6_params.G6_plot_params(run_dir, epoch, params, params_ML, param_file_ML, -1)
        else:
            g6_params.G6_plot_params(run_dir, epoch, params, params_ML_best, param_file_ML_best, -1)
    ...
    for epoch in range(1,n_epoch):
        ...
        if epoch % plot_every == 0:
            ...
            if os.path.exists(param_file_ML):
                ...
                g6_params.G6_plot_params_alone(run_dir, epoch, params)
...
def main(args):
    ...
    with os.scandir(run_dir) as it:
        if any(it): shutil.rmtree(run_dir)
    run_dir.mkdir(parents=False, exist_ok=True)
    ...
    optimize_param_G6(args, run_dir, K, mask, psq, log_psq, grad_accum, acc_ymin, acc_ymax, verbose)
