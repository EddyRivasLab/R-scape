...
import  grammars.g5.g5_params as g5_params
...
def G5_inside_calculate(args, rundir, K, mask, psq, psq2, log_psq, log_psq2, psq_name, verbose):
    ...
    scale       = K
    if scaled: print("G5 params (scaled)")
    else:      print("G5 params (logsumexp)")

    # the parameters
    if   param_tornado: log_t, t, e_single, pe_single, e_pair, pe_pair = g5_params.G5_param_tornado(verbose)
    elif param_naive:   log_t, t, e_single, pe_single, e_pair, pe_pair = g5_params.G5_param_naive(verbose)
    ...
    for i in range(len(psq)):
        ...
        #p_inside_jax_scaled = g5_inside_jax_scaled(mask_i, psq_i, psq2_i, t, pe_single, pe_pair)                                    # scaled jax
        #logp_inside_std_scaled = G5_Inside_std_scaled(mask_i, psq_i, psq2_i, t, pe_single, pe_pair, scale, verbose, K, min_hairpin) # scaled standard
        #logp_inside_jax_scaled = np.log(p_inside_jax_scaled) - n * np.log(scale)
        
        logp_inside_jax = g5_inside_jax(mask_i, log_psq_i, log_psq2_i, log_t, e_single, e_pair)                          # logspace jax
        #logp_inside_std = G5_Inside_std(mask_i, log_psq_i, log_psq2_i, log_t, e_single, e_pair, verbose, K, min_hairpin) # logspace standard
...
def main(args):
    ...
    # calculate inside
    G5_inside_calculate(args, rundir, K, mask, psq, psq2, log_psq, log_psq2, psq_name, verbose)
