...
def loss_fn_logs(g5_inside_jax, mask, log_psq, log_psq2, log_t, e_single, e_pair):
...
def loss_fn_probs(g5_inside_jax, mask, psq, psq2, t, pe_single, pe_pair):
...
def optimize_param_G5(args, rundir, K, mask, psq, log_psq, verbose):
    ...
    def loss_fn(unprocessed_params, mask, psq, psq2):
        params = process_params_g5(unprocessed_params)
        
        if scaled:
            loss = loss_fn_probs(g5_inside_jax, mask, psq, psq2, params['t'], params['pe_single'], params['pe_pair'])
        else:
            log_psq  = jnp.log(psq)
            log_psq2 = jnp.log(psq2)
            
            loss = loss_fn_logs(g5_inside_jax, mask, log_psq, log_psq2, params['log_t'], params['e_single'], params['e_pair'])
    ...
    for epoch in range(n_epoch):
        ...
        processed_params = process_params_g5(params)
