...
def test_G5S_Inside_JAX(n: int, single_seq: int, verbose: int, K: int=4, min_hairpin: int = 0):
    epsilon = 1e-3

    scale = K

    log_t, t, e_single, pe_single, e_pair, pe_pair, e_stck, pe_stck = G5S_param_tornado(verbose)

    g5s_inside_jax_scaled = G5S_Inside_JAX_scaled(scale, verbose, K, min_hairpin)
    g5s_inside_jax        = G5S_Inside_JAX(              verbose, K, min_hairpin)
    ...
    for seq in range(nseqs+1):
        ...
        assert np.allclose(
            logp_inside_std, logp_inside_std_scaled, atol=1e-5), f"inside_std={logp_inside_std} inside_std_scaled={logp_inside_std_scaled}"
        print(f"Test log_std/scaled_std passed", logp_inside_std, logp_inside_std_scaled)
...
def main():
    ...
    print("test G5S inside")
    test_G5S_Inside_JAX(n, single_seq, verbose, K=4, min_hairpin=0)
    test_G5S_Inside_JAX(n, single_seq, verbose, K=4, min_hairpin=3)
